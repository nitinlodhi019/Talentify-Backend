...
def calculate_match_score_enhanced(job_description_text, required_skills, experience_required,
                                   resume_processed_text, resume_extracted_skills):
    ...
    if skill_match_percentage > 0.7:
        skill_match_percentage *= 1.2
    elif skill_match_percentage < 0.3:
        skill_match_percentage *= 0.85
    ...
    if experience_required and experience_required != "Any":
        ...
        if resume_experience_match:
            ...
            level_keywords = ["senior", "junior", "entry-level", "lead", "manager"]
            matched_level = next((kw for kw in level_keywords if kw in job_desc_lower and kw in resume_text_lower), None)
            experience_score = 0.9 if matched_level in ("senior", "junior", "entry-level") else \
                               0.85 if matched_level == "lead" else \
                               0.8 if matched_level == "manager" else 0.6
